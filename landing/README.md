...
The homepage is a short brand-and-product introduction: a black hero with the official metal shield, a white section containing one private/public pricing example, and a black download close. The original detailed interactive tour is retained at `how-it-works.html`.
...
| `landing.css` | Homepage layout, black/white surfaces, components and responsive behavior. |
...
- The shield turns through a finite 4.8-second entrance, then settles. Fine-pointer interaction changes its pose and reflections. Rendering stops when settled, offscreen or hidden. Motion preference changes and WebGL loss are handled by the existing controller.
- Reduced-motion, low-capability and no-JavaScript visitors retain the static official shield. Film media loads only after deliberate activation. Closing the dialog pauses video and returns focus.
