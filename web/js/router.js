...
export function parseHash() {
  ...
  return {
    // An unaddressed launch begins with the product's main action. Explicit
    // links still open exactly the list, record or setup screen they name.
    view: VIEWS[view] ? view : 'policy',
    sel: VIEWS[view] ? (rest.length ? decodeURIComponent(rest.join('/')) : null) : 'new',
    query
  };
}
