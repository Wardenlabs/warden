...
test('current HTML still exposes the instrumented native controls and known assets', async () => {
  ...
  assert.match(html, /"softwareVersion": "0\.2\.18"/);
  assert.match(html, /"releaseNotes": "https:\/\/github\.com\/Wardenlabs\/warden\/releases\/tag\/v0\.2\.18"/);
  assert.equal((html.match(/v0\.2\.18 · Free and open source\./g) || []).length, 2);
