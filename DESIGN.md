...
**Download.** Off-white pill with dark text and an operating-system icon; minimum height 48px. Hover changes to light gray on fine pointers. The hero inherits a small pressed translation/scale from the shared button. Footer downloads are grouped text links; the primary uses 20–32px type and a downward icon in a thin circular outline, without repeating the hero pill. Direct links work before enhancement; platform detection updates the primary installer and footer ordering. Preserve existing installer targets and hooks.
